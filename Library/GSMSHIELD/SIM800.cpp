...
const short gsm_oper_num[]=
	{
	MCCMNC_none,
	//Russia
	MCCMNC_mts,		//MTS
	MCCMNC_megafon,	//Megafon
	MCCMNC_aline_gsm,	//НСС, Aline-gsm, «Ростелеком»
	MCCMNC_etk,		//ЕТК, «Ростелеком»
	MCCMNC_smarts,		//СМАРТС
	MCCMNC_bwc,		//Байкалвестком, «Ростелеком»
	MCCMNC_ntc,		//НТК
	MCCMNC_Utel,		//Utel, «Ростелеком»
	MCCMNC_tele2,		//Tele2
	MCCMNC_motiv,		//motiv
	MCCMNC_tmb,		//Тамбов GSM, «Ростелеком»
	MCCMNC_ogsm,		//Оренбург-GSM, «Ростелеком»
	MCCMNC_beeline,	//Beeline
	///Belarus
	MCCMNC_velcom_by,
	MCCMNC_mts_by,
	MCCMNC_life_by
...
char SIMCOM800::forceON()
{
     ...
     *(p_char1+2)=0;
     ...
     if((*p_char1)=='4') {
		...
		_cell.PowerOn(true);	//Power On and Press key
          delay(1200);
          //digitalWrite(GSM_ON, LOW);
		_cell.PowerOn(false);	//Key unpress
          delay(10000);
          ret_val=2;
     }
...
int SIMCOM800::configandwait(char* pin)
{
     int connCode;
...
/**
 * SIMCOM800::read(char* buffer, int buffersize)
 *
 ...
 *
 */
int SIMCOM800::read(char* result, int resultlength)
{
char temp;
int i=0;
...
/*******************************************************************************
 * SIMCOM800::read(char* buffer, int buffersize, int timeout)
 *
 * Waits for data to be readable from the gsm module, reads data until
 * no more is available or the buffer has been filled
 *
 ...
 *
 ******************************************************************************/
int SIMCOM800::read(char* result, int resultlength, int timeout)
{
char temp;
int i=0;
...
while(gsm.available()<1)
	{
	delay(100);
	if ((prev_time + timeout) < millis()) return 0;
     }
...
/*
http://mobile.maps.yandex.net/cellid_location/?cellid=1341354&operatorid=01&countrycode=250&lac=13109

*/
bool SIMCOM800::readCellData(int &mcc, char &mnc, int &lac, int &cellid, char &rxl)
{
if (getStatus()==IDLE) return false;

//_tf.setTimeout(_GSM_DATA_TOUT_);
_cell.flush();
SimpleWriteln("AT+CENG=3,0");
gsm.WaitResp(5000, 50, "OK");
SimpleWriteln("AT+CENG?");
uint32_t prev_time = millis();
while (gsm.available()<60) if ((prev_time + 5000) < millis()) return false;

char str_num[23]={};
if (_cell.getString("+CENG: 0,\"", "\"\r\n", str_num, 23))
	{
	mcc=atoi(&str_num[0]);
	mnc=atoi(&str_num[4]);
	lac=StringToHex(&str_num[7], 4);
	cellid=StringToHex(&str_num[12], 4);
	rxl=atoi(&str_num[20]);
	}
else return false;
WaitResp(1000, 100, "OK");
return true;
}



char SIMCOM800::readCellData(int *mcc, char *mnc, int *lac, int *cellid, char *rxl)
{
...
char str_num[23]={};
char start_str[11]="+CENG: 0,\"";

uint32_t prev_time = millis();
RxInit(1000, 100);
while (available()<60);// if ((prev_time + 3000) < millis()) return 0;
...
while(1)
	{
	start_str[7]=i|0x30;
	//if (_cell.getString(start_str, "\"\r\n", str_num, 23))
	if (IsLineReceived())
		{
		LineReceiv(str_num);
		if (IsStringReceived("OK\r\n")) break;
		mcc[i]=atoi(&str_num[0]);
		mnc[i]=atoi(&str_num[4]);
		lac[i]=StringToHex(&str_num[7], 4);
		cellid[i]=StringToHex(&str_num[12], 4);
		rxl[i]=atoi(&str_num[20]);
		//RxInit(1000, 100);
		}
	//else break;
	prev_time = millis();
	//while (available()<36) if ((prev_time + 1000) < millis()) return i;
	}
...
TEopsos SIMCOM800::GetOpsos(short mcc, char mnc)
{
int code=(int)mcc*100+mnc;
for (uint8_t i=sizeof(gsm_oper_num)/2; i>0; i--)
	{
	if (code==gsm_oper_num[i]) return (TEopsos) i;
	}
return opsos_none;
}
...
boolean SIMCOM800::readSMS(char* msg, int msglength, char* number, int nlength)
{
     //Serial.println(F("This method is deprecated! Please use GetSMS in the SMS class."));
#ifdef DEBUG_ON
printf("This method is deprecated! Please use GetSMS in the SMS class.");
#endif
long index;
     char *p_char;
     char *p_char1;
...
boolean SIMCOM800::readCall(char* number, int nlength)
{
     //int index;
...
boolean SIMCOM800::call(char* number, unsigned int milliseconds)
{
     if (getStatus()==IDLE)
          return false;
     ...
     SimpleWrite(number);
     SimpleWriteln(F(";"));
     for (uint16_t i=0; i<milliseconds; i++) delay(1);
     SimpleWriteln(F("ATH"));
...
int SIMCOM800::changeNSIPmode(char mode)
{
...
int SIMCOM800::getIMEI(char *imei)
{
//Expect str_ok.
if(gsm.WaitResp(5000, 50, str_ok)!=RX_FINISHED_STR_NOT_RECV)
	{
...
bool SIMCOM800::IsDataMode()
{
return _cell.IsDataMode();
}



void SIMCOM800::SetDataMode()
{
return _cell.SetDataMode();
}



void SIMCOM800::SetCmdMode()
{
return _cell.SetCmdMode();
}



void SIMCOM800::SimpleRead()
{
char datain;
...
size_t SIMCOM800::SimpleWrite(char *comm)
{
return _cell.print(comm);
}



size_t SIMCOM800::SimpleWrite(const char *comm)
{
return _cell.print(comm);
}
...
void SIMCOM800::SimpleWrite(int comm)
{
_cell.print(comm);
}
...
size_t SIMCOM800::SimpleWriteln(char *comm)
{
return _cell.println(comm);
}



size_t SIMCOM800::SimpleWriteln(char const *comm)
{
return _cell.println(comm);
}
...
void SIMCOM800::SimpleWriteln(int comm)
{
     _cell.println(comm);
}
...
void SIMCOM800::SimpleWriteArray(void * array, int len)
{
_cell.write((uint8_t *)array, len);
}



void SIMCOM800::WhileSimpleRead()
{
char datain;
...
bool GSM::IsRegistered(void)
{
     return (module_status & STATUS_REGISTERED);
}

bool GSM::IsInitialized(void)
{
...
return values:
      ...
      REG_COMM_LINE_BUSY  - comm line between GSM module and Arduino is not free
                            for communication
**********************************************************/
TEregistration_ret_val GSM::CheckRegistration(void)
{
     byte status;
     TEregistration_ret_val ret_val = REG_NOT_REGISTERED;
...
signed char SIMCOM800::GetSignalQuality()
{
if (CLS_FREE != GetCommLineStatus()) return (REG_COMM_LINE_BUSY);
SetCommLineStatus(CLS_ATCMD);
signed char n=0;

SimpleWriteln(F("AT+CSQ"));
if(gsm.WaitResp(500, 50, str_ok)!=RX_FINISHED_STR_NOT_RECV)
	{
	char *p = strstr((char *)this->comm_buf, "+CSQ: ");
	n = atoi(p + 5);
...
unsigned char SIMCOM800::GetBattery()
{
if (CLS_FREE != GetCommLineStatus()) return (REG_COMM_LINE_BUSY);
SetCommLineStatus(CLS_ATCMD);
signed char n=0;
...
RxInit(500, 50);
SimpleWriteln(F("AT+CBC"));
if(WaitResp(1000, 100, str_ok)==RX_FINISHED_STR_RECV)
	{
	char *p = strstr((char *)this->comm_buf, "+CBC: 0,");
	n = atoi(p + 8);
     }
else n=0;
...
unsigned char SIMCOM800::GetTemperature()
{
if (CLS_FREE != GetCommLineStatus()) return (REG_COMM_LINE_BUSY);
SetCommLineStatus(CLS_ATCMD);
signed char n=0;
...
RxInit(500, 50);
SimpleWriteln("AT+CMTE?");
if(WaitResp(1000, 100, str_ok)==RX_FINISHED_STR_RECV)
	{
	char *p = strstr((char *)this->comm_buf, "+CMTE: 0,");
	n = atoi(p + 9);
     }
else n=0;
...
void SIMCOM800::PowerDown()
{
SetCommLineStatus(CLS_ATCMD);
flush();
SimpleWriteln("AT+CPOWD=1");
WaitResp(2000, 50, "NORMAL POWER DOWN");
delay(500);
SetCommLineStatus(CLS_FREE);
_cell.PowerOn(false);
}
