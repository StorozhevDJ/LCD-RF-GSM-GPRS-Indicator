...
typedef enum rx_state_enum {
	...
	RX_FINISHED_LINE_RECV,	// line received
     RX_TMOUT_ERR,			// finished, no character received
	RX_FINISHED_BUF_FULL,	// finished, buffer is full
...
public:
	...
	byte IsLineReceived(void);
	char LineReceiv(char * line);
	...
	uint16_t GetRxData(uint8_t  *data,
				    uint8_t  data_len,
				    uint16_t tmout);
     ...
     TEregistration_ret_val CheckRegistration(void);
