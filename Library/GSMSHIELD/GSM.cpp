...
for (cont=0; cont<1; cont++)
	{
	if ((char)AT_RESP_ERR_NO_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)&&!turnedON)
		{//check power
...
if(norep==true&&!turnedON)
	{
#ifdef DEBUG_ON
	printf("Trying to force the baud-rate to 9600\n");
#endif
	for (int i=0; i<8; i++)
		{
		switch (i)
...
#ifdef DEBUG_ON
	printf("ERROR: SIM900 doesn't answer! Check power!\r\n");
#endif
...
void GSM::InitParam(byte group)
{
     ...
     case PARAM_SET_0:
		...
		SendATCmdWaitResp(F("AT+IFC=2,2"), 500, 50, str_ok, 5);
     ...
     case PARAM_SET_1:
		...
		//Set transparent mode (0 = non-transparent mode)
          SendATCmdWaitResp("AT+CIPMODE=1", 500, 50, str_ok, 5);
...
do	{
	status = (TErx_state)IsRxFinished();
...
/*uint16_t GSM::GetRxData(uint8_t  *data,
				    uint8_t  data_len,
				    uint16_t tmout)
{
//TErx_state status;
...
uint8_t data_ptr=0;
uint32_t timeout=millis()+tmout;

while (millis()<timeout)
	{
	if (_cell.available())
		{
		data_ptr+=_cell.read(data[data_ptr], data_len-data_ptr, tmout);
		}
	if (data_len==data_ptr) return data_ptr;
	}
return data_ptr;*/
                            ...
                            byte no_of_attempts)
{
     ...
     for (i = 0; i < no_of_attempts; i++) {
          ...
          status = (TErx_state) WaitResp(start_comm_tmout, max_interchar_tmout);
          if (status == RX_FINISHED) {
               // something was received but what was received?
               // ---------------------------------------------
...
if (rx_state == RX_NOT_STARTED)
	{
	// Reception is not started yet - check tmout (if input buffer empty)
	if (!_cell.available())
		{
...
if (rx_state == RX_ALREADY_STARTED)
	...
	// check new received bytes only in case we have place in the buffer
	...
	while (num_of_bytes)
		...
		if (comm_buf_len < COMM_BUF_LEN)
			...
			*p_comm_buf = _cell.read();
			...
			// so after each character we have valid string finished by the 0x00
			}
		else
			{
	...
	while (num_of_bytes)
		...
		if (comm_buf_len < COMM_BUF_LEN)
			...
			*p_comm_buf = _cell.read();
			...
			p_comm_buf++;
			comm_buf_len++;
			*p_comm_buf=0;//comm_buf[comm_buf_len] = 0x00;  // and finish currently received characters
			if (comm_buf[comm_buf_len-1]=='\n') return RX_FINISHED_LINE_RECV;
			// so after each character we have valid string finished by the 0x00
			}
		else	return RX_FINISHED_BUF_FULL;// comm buffer is full
		}
...
char GSM::LineReceiv(char * line)
{
while (1)
	{
	byte lr=IsLineReceived();
	if ((lr==RX_TMOUT_ERR)||(lr==RX_FINISHED_BUF_FULL)) return 0;
	if (lr==RX_FINISHED_LINE_RECV) break;
	}
strcpy(line, (const char *)comm_buf);

byte ret=comm_buf_len;
...
if(comm_buf_len)
	...
	{
#ifdef DEBUG_ON
	printf("ATT: %s RIC: NO STRING RCVD\r\n", compare_string);
#endif
     }
return (ret_val);
}
